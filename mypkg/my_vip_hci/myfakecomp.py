...
from vip_hci.preproc import cube_crop_frames, frame_shift, frame_crop, \
    cube_shift
from vip_hci.var import frame_center, fit_2dgaussian, fit_2dairydisk, \
    fit_2dmoffat, get_circle, get_annulus_segments
...
def _cube_inject_adi(array: ndarray, psf_template: ndarray, angle_list: ndarray,
    ...
    posns_y = ceny + np.outer(rad_dists, np.sin(angs)).reshape(-1)
    posns_x = cenx + np.outer(rad_dists, np.cos(angs)).reshape(-1)
    positions = np.stack((posns_y, posns_x), axis=1)

    # TODO check that the dimensions of all the arrays aren't wonky
    ...
    for branch in range(n_branches):
        ...
        for i, rad in enumerate(rad_dists):
            ...
            shifts_y = rad * np.sin(ang - np.deg2rad(angle_list))
            shifts_x = rad * np.cos(ang - np.deg2rad(angle_list))
            ...
            dsy = shifts_y-int(shifts_y)
            dsx = shifts_x-int(shifts_x)
            fc_fr_rad = fc_fr.copy()
            # this can be MP'd
            if nproc == 1:
                for fr in range(nframes):
                    fc_fr_rad = frame_shift(fc_fr_rad[fr], dsy[fr], dsx[fr],
                        imlib, interpolation, border_mode='constant')
                    array_sh[fr, y0[fr]:yN[fr], x0[fr]:xN[fr]] = \
                        flevel[fr]*fc_fr_rad[p_y0[fr]:p_yN[fr], p_x0[fr]:p_xN[fr]]
            # TODO figure out how to MP and good indexing technique
            else:
                array_sh[:, y0:yN, x0:xN] = pool_map(nproc, frame_shift, iterable(fc_fr_rad,...))
            array_out += array_sh
...
def _psf_norm_2d(psf, fwhm, threshold, mask_core, full_output, verbose,
                    fit_2d, imlib="vip-fft", interpolation="lanczos4"):
