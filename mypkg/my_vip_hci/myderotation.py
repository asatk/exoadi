...
from vip_hci.preproc.cosmetics import frame_pad
...
def _find_indices_adi(angle_list, frame, thr, nframes=None, out_closest=False,
    ...
    where_prev = np.abs(angle_list[frame] - angle_list[:frame]) < thr
    index_prev = np.argmax(where_prev)
    if index_prev == 0 and np.argmin(where_prev):
        index_prev = frame

    where_foll = np.abs(angle_list[frame] - angle_list[frame:]) > thr
    index_foll = np.argmax(angs_abs_foll > thr)
    if index_foll == 0 and np.argmin(where_foll):
        index_foll = n
